...
def temperature():
    ...
    most_recent1 = most_recent[0]
    most_recent2 =  dt.datetime.strptime(most_recent1, "%Y-%m-%d") 
    most_recent_prev = most_recent2- dt.timedelta(days=365)
    ...
    temp_data_12_months = session.query(Measurement.date, Measurement.tobs).\
        filter(Measurement.date <= most_recent2).\
        filter(Measurement.date >= most_recent_prev).\
        order_by(Measurement.date).all()
